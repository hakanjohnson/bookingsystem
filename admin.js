...
onAuthStateChanged(auth, (user ) => {
  if(user) {
    ...
    document.getElementById("greet").textContent = "Hallo, " + email;
  }
})
...
onSnapshot(collection(db, "termine"), (snapshot) => {
  ...
  snapshot.forEach((doc) => {
      ...
      div.style.border = "3px solid #3bec38";
...
//Kurse anzeigen
const containerKurse = document.getElementById("kurse");
onSnapshot(collection(db, "kurse"), (snapshot) => {
  containerKurse.innerHTML = "";
  snapshot.forEach((doc) => {
      const data = doc.data();
      const div = document.createElement("div");
      div.style.border = "3px solid #253bb9";
      div.style.borderRadius = "6px";
      div.style.margin = "10px";
      div.style.padding = "10px";

      div.innerHTML = `
      <strong>Termin:</strong> ${data.title} <br>
      <strong>Datum:</strong> ${data.date} <br>
      <strong>Zeit:</strong> ${data.time} <br>
      <strong>Plätze:</strong> ${data.kapazität} <br>
      `;
      containerKurse.appendChild(div);
  });
});
